...
export class QuizAttemptService {
  ...
  async submitAnswer(data: SubmitAnswerData) {
    try {
      ...
      // Update attempt with correct answers count
      if (isCorrect) {
      ...
      return {
        answer,
        isCorrect,
        correctOptionIds: isCorrect ? undefined : correctOptionIds // Only show correct answers if wrong
      };
    } catch (error) {
  ...
  async completeQuizAttempt(data: CompleteQuizAttemptData) {
    try {
      ...
      const totalTimeSpent = attempt.answers.reduce((sum: number, answer: any) => 
      ...
      const completedAttempt = await prisma.quizAttempt.update({
        ...
        include: {
          ...
          answers: {
            include: {
              attempt: false // Avoid circular reference
            }
          }
        }
      });

      // Update user stats (total matches, wins if score >= 70%)
    ...
    try {
      const attempt = await prisma.quizAttempt.findUnique({
        ...
        include: {
          ...
          answers: {
            include: {
              // We'll need to join with question bank items here
            }
          }
        }
      });
  ...
  async getLeaderboard(quizId?: number, limit = 10) {
    try {
      ...
      return topAttempts.map((attempt: any, index: number) => ({
        rank: index + 1,
        user: attempt.user,
        quiz: attempt.quiz,
        score: attempt.score,
  ...
  async getUserStats(userId: number) {
    try {
      ...
      const averageScore = completedAttempts > 0 
        ? attempts.reduce((sum: number, a: any) => sum + a.score, 0) / completedAttempts 
        : 0;

      const difficultyStats = {
        EASY: attempts.filter((a: any) => a.quiz.difficulty === 'EASY').length,
        MEDIUM: attempts.filter((a: any) => a.quiz.difficulty === 'MEDIUM').length,
        HARD: attempts.filter((a: any) => a.quiz.difficulty === 'HARD').length
      };
      ...
      return {
        ...
        stats: {
          ...
          totalTimeSpent: attempts.reduce((sum: number, a: any) => sum + (a.timeSpent || 0), 0),
          difficultyStats
        },
        recentAttempts
      };
