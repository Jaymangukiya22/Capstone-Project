...
exports.EngineeringSeeder = void 0;
...
class EngineeringSeeder {
    async run() {
        try {
            ...
            (0, logger_1.logError)('Seeding failed', error);
            throw error;
        }
    }
            ...
            for (const subCat of rootCat.children) {
                ...
                if (subCat.children) {
                    for (const subSubCat of subCat.children) {
                        const subSubCategory = await models_1.Category.create({
                            name: subSubCat.name,
    ...
    async createQuestionsAndOptions(categories, users) {
        console.log('❓ Creating engineering questions and options...');
        const questions = [];
        const options = [];
        const adminUsers = users.filter(u => u.role === models_1.UserRole.ADMIN);
        ...
        for (const questionSet of ENGINEERING_QUESTIONS) {
            ...
            for (const q of questionSet.questions) {
                ...
                for (let i = 0; i < 4; i++) {
                    if (q.options[i]) {
                        const option = await models_1.QuestionBankOption.create({
    ...
    async createQuizzes(categories, users) {
    ...
    async assignQuestionsToQuizzes(quizzes, questions) {
    ...
    async createSampleAttempts(users, quizzes) {
        console.log('🎯 Creating sample quiz attempts...');
        const studentUsers = users.filter(u => u.role === models_1.UserRole.ADMIN);
        const attempts = [];
        for (let i = 0; i < 50; i++) {
...
exports.EngineeringSeeder = EngineeringSeeder;
...
exports.default = EngineeringSeeder;
//# sourceMappingURL=engineeringSeeder.js.map